...
- [ ] When you use oath to login and press cancel or any other type of failure the pop up will redirect to the normal page inside the pop up so you will have two pages with the board. It should close and send an error toast.
- [ ] When using oauth if the user clicks any option and closes the pop up and then clicks any other options (repeat x times) it will show the toast message for the corresponding option x times Ex. i press auth with google and close the window three times and then auth with github, It will show 3 toast messages of successful auth with google and one with guthub
- [ ] 
